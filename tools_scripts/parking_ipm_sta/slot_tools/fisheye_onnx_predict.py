...
def detect_fisheye(fisheye_img, img_name, onnx_mode_path, args):
    fisheye_config = args.img_path
    calib_data_path = args.calib_data_path
    # mask_front_path = os.path.join(fisheye_config, 'mask_res/0_mask_89.jpg')
    # mask_left_path = os.path.join(fisheye_config, 'mask_res/1_mask_89.jpg')
    # mask_back_path = os.path.join(fisheye_config, 'mask_res/2_mask_89.jpg')
    # mask_right_path = os.path.join(fisheye_config, 'mask_res/3_mask_89.jpg')
    mask_root = os.path.join(calib_data_path, 'mask')
    mask_front_path = os.path.join(mask_root, 'mask_front_89.jpg')
    mask_left_path = os.path.join(mask_root, 'mask_left_89.jpg')
    mask_back_path = os.path.join(mask_root, 'mask_back_89.jpg')
    mask_right_path = os.path.join(mask_root, 'mask_right_89.jpg')
    ...
    # img_rear = cv2.cvtColor(img_rear, cv2.COLOR_BGR2RGB)
    ...
    # np.savetxt(data_path_for_avm_onnx + '/mask_back.txt', np.array(mask_back, np.uint8))
    mask_front = cv2.imread(mask_front_path,0)
    # mask_front = cv2.resize(mask_front, (avm_w,avm_h))
    # cv2.imwrite('/media/gpal/mnt1/eddie/code/ParkingSlot_detect/parking_slot/tools/maskfront_512.jpg',mask_front)
    # np.savetxt(data_path_for_avm_onnx + '/mask_front.txt', np.array(mask_front, np.uint8))
    mask_left = cv2.imread(mask_left_path,0)
    # mask_left = cv2.resize(mask_left, (avm_w,avm_h))
    # cv2.imwrite('/media/gpal/mnt1/eddie/code/ParkingSlot_detect/parking_slot/tools/maskleft_512.jpg',mask_left)
    # np.savetxt(data_path_for_avm_onnx + '/mask_left.txt', np.array(mask_left, np.uint8))
    mask_right = cv2.imread(mask_right_path,0)
    # mask_right = cv2.resize(mask_right, (avm_w,avm_h))
    # cv2.imwrite('/media/gpal/mnt1/eddie/code/ParkingSlot_detect/parking_slot/tools/mask_right_512.jpg',mask_right)
    # np.savetxt(data_path_for_avm_onnx + '/mask_right.txt', np.array(mask_right, np.uint8))


    rear_front_path = os.path.join(calib_data_path, 'mask/grid_rear_and_front.npy')
    left_right_path = os.path.join(calib_data_path, 'mask/grid_left_and_right.npy')
    ...
    # onnx_mode_path = "/home/gpal/gpal_work/ParkingSlot/parking_slot/master_main/model_pool/20250714-143557_ddrnet_slim23_model_50_fisheye.onnx"
    # onnx_model = onnx.load('/home/gpal/gpal_work/ParkingSlot/parking_slot/master_main/model_pool/20250714-143557_ddrnet_slim23_model_50_fisheye.onnx')
    # file_name = onnx_mode_path.split('/')[-1].split('.')[0] + "_onnx_res/" + img_name
...
def main(args):
    ...
    for pic_folder in test_pic_folder:
        ...
        for img_file in img_file_list:
            ...
            model_h = 768
            model_w = 768
            detect_fisheye(fisheye_img, img_file, onnx_mode_path, args)
