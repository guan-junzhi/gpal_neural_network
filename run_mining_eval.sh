...
rm -rf workspace/dyn_pth_eval_mining_62
mkdir -p workspace/dyn_pth_eval_mining_62

# Use 1 GPU since only 60 frames
python3 eval.py \
  --load_from /data/ai_group/workdirs/gpal_neural_network_group/airflow_workspace/detzero_one_node_traning_job_on_airflow_for_k8s_20260409_14_56_14/checkpoint/epoch=1-step=10000_checkpoint.pth \
  --save workspace/dyn_pth_eval_mining_62 \
  --config configs_for_develop/dyn_fusion_eval_detzero_test.yaml \
  --gpus 1 \
  > workspace/dyn_pth_eval_mining_62/log.txt 2>&1
