...
class Point_Feature_Net(BaseModule):
    def __init__(self, global_config, point_process_config, reader_config,scatter_config):
        # super(Resnet, self).__init__()
        super(Point_Feature_Net, self).__init__(global_config)
        max_voxels_num = tuple(point_process_config['max_voxels_num'])
        num_filters = tuple(reader_config['num_filters'])
        pool_size = tuple(reader_config['pool_size'])
        
        self.pre_process = CenterPointPreProcess(
            pc_range=point_process_config['pc_range'],
            voxel_size=point_process_config['voxel_size'],
            max_voxels_num=max_voxels_num,
            max_points_in_voxel=point_process_config['max_points_in_voxel'],
            norm_range=point_process_config['norm_range'],
            norm_dims=point_process_config['norm_dims'],
        )
        self.reader = PillarFeatureNet(
            num_input_features=reader_config['num_input_features'],
            num_filters=num_filters,
            with_distance=reader_config['with_distance'],
            voxel_size=reader_config['voxel_size'],
            pc_range=reader_config['pc_range'],
            quantize=reader_config['quantize'],
            use_4dim=reader_config['use_4dim'],
            use_conv=reader_config['use_conv'],
            pool_size=pool_size,
            normalize_xyz=reader_config['normalize_xyz'],
            hw_reverse=reader_config['hw_reverse'],
        )
        self.scatter = PointPillarScatter(
            num_input_features=scatter_config['num_input_features'],
            use_horizon_pillar_scatter=scatter_config['use_horizon_pillar_scatter'],
        )
...
if __name__ == "__main__":
    ...
    model = Point_Feature_Net(global_config, point_process_config, reader_config, scatter_config)
    model.eval()
    input = dict(
