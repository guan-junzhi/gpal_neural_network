...
# from models.hat.utils.dtype import qint16
# from horizon_plugin_pytorch.nn.quantized import FloatFunctional as FF
# from models.hat.utils.utils import QuantStub
from torch import Tensor, nn

from gpal_nn.models.transformers.bevformer.utils import constant_init, xavier_init, QuantStub
...
class MultiScaleDeformableAttentionBase(nn.Module):
    ...
    def get_sampling_locations(
        ...
        if not self.deploy:
            ...
            sampling_offsets_tmp = torch.mul(
                sampling_offsets, offset_normalizer[None, None, None, :]
            )
            sampling_locations = torch.add(
                reference_points, sampling_offsets_tmp
            )
            sampling_locations = torch.add(
                torch.mul(sampling_locations, 2), -1
            )
        else:
            reference_points = torch.add(
                torch.mul(reference_points, 2), -1
            )
            reference_points = reference_points.repeat(
                1, 1, self.num_heads, self.num_points
            ).reshape(
    ...
    def multi_scale_deformable_attn_pytorch(
        ...
        if to_onnx:
            value_spatial_shapes = [[value_spatial_shapes[0][0], value_spatial_shapes[0][1]]]
        bs, _, num_heads, embed_dims = value.shape
        _, num_queries, num_heads, _ = sampling_locations.shape
        if to_onnx:
            value_list = value.split([value_spatial_shapes[0][0] * value_spatial_shapes[0][1]], dim=1)
        else:
        ...
        sampling_value_list_outs = torch.cat(
            sampling_value_list, dim=-1
        )

        sampling_value_list_outs = torch.mul(
    ...
    def forward(
        ...
        **kwargs,
        ...
        output = self.multi_scale_deformable_attn_pytorch(
            value, spatial_shapes, sampling_locations, attention_weights, to_onnx
        )
        return output
    ...
    def forward(
        ...
        **kwargs,
        ...
        if query_pos is not None:
            query = torch.add(query, query_pos)
        ...
        output = torch.add(self.dropout(output), identity)
...
class HorizonTemporalSelfAttention(MultiScaleDeformableAttentionBase):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query = torch.cat([prev_value, query], -1)
...
class HorizonSpatialCrossAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        queries = torch.add(self.dropout(slots), inp_residual)
        return queries
...
class HorizonMultiPointDeformableAttention(MultiScaleDeformableAttentionBase):
    ...
    def get_sampling_locations(
        ...
        if not self.deploy:
            ...
            sampling_locations = torch.mul(sampling_locations, 2) - 1
    ...
    def forward(
        ...
        **kwargs,
        ...
        output = (
            super()
            .forward(query, value, reference_points, spatial_shapes, to_onnx, **kwargs)
            .transpose(1, 2)
            .contiguous()
        )
        output = self.output_proj(output)
...
class DetrTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        query = torch.add(query, self.dropout1(sa_query))
