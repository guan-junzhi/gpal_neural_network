...
from gpal_nn.models.transformers.bevformer.encoder import SingleBEVFormerEncoder, SingleBEVFormerEncoderLayer
...
class BevFormerViewTransformer(BaseModule):
    ...
    def __init__(self, global_config, transformer_config, freeze_module: bool = False):
        ...
        bev_h = transformer_config['bev_y']
        bev_w = transformer_config['bev_x']
        embed_dims = transformer_config['embed_dims']
        queue_length = transformer_config['queue_length']
        max_camoverlap_num = transformer_config['max_camoverlap_num']
        # virtual_bev_x = transformer_config['virtual_bev_x']
        # virtual_bev_y = transformer_config['virtual_bev_y']
        numcam = transformer_config['numcam']
        in_indices = (-1,)
        is_compile = False
        single_bev = transformer_config['single_bev']
        use_lidar2img = transformer_config['use_lidar2img']
        ...
        self.positional_encoding = PositionEmbeddingLearned(
            num_pos_feats=embed_dims // 2,
            row_num_embed=bev_h,
            col_num_embed=bev_w,
        )
        self.encoder = SingleBEVFormerEncoder(
            bev_h=bev_h,
            bev_w=bev_w,
            ...
            encoder_layer=SingleBEVFormerEncoderLayer(
                embed_dims=embed_dims,
                dropout=0.1,
                selfattention=HorizonMultiScaleDeformableAttention(
                    embed_dims=embed_dims,
                    num_heads=8,
                    num_levels=1,
                    num_points=4,
                    ...
                    batch_first=True,
                    grid_align_num=10,
                    feats_size=[[bev_w, bev_h]],
                ),
                crossattention=HorizonSpatialCrossAttention(
                    bev_h=bev_h,
                    bev_w=bev_w,
                    embed_dims=embed_dims,
                    num_cams=numcam,
                    ...
                    deformable_attention=HorizonMultiScaleDeformableAttention3D(
                        embed_dims=embed_dims,
                        num_heads=8,
                        num_points=8,
                        num_levels=1,
        ...
        self.virtual_bev_h = bev_h
        self.virtual_bev_w = bev_w
    ...
    def point_sampling(
        ...
        # ref3d_vis = reference_points[0,0].reshape(-1,3)
        # vis = Vis2D([-30, 100], [-40, 40], 0.1)
        # # vis.DrawPointcloud(ref3d_vis.cpu().numpy()[30:], [0,255,0])
        # # vis.DrawPointcloud(ref3d_vis.cpu().numpy()[:30], [255,0,0])
        # vis.DrawPointcloud(ref3d_vis.cpu().numpy(), [0,255,0])
        # vis_draw = vis.Draw()
        # cv2.imwrite("reference_points.jpg", vis_draw)
        # # exit(1)
        ...
        # import cv2
        # ele_i = 0
        # reference_points_cam[...,0] = reference_points_cam[...,0]* 640
        # reference_points_cam[...,1] = reference_points_cam[...,1]* 196
        # for cam_id in range(3):
        #     for batch_id in range(5):
        #         img = np.zeros([196, 640])
        #         reference_points_cam = reference_points_cam.int()

        #         ele3, ele4 = bev_mask[0,batch_id, cam_id, ...], reference_points_cam[0,batch_id, cam_id, ...]
        #         pts = ele4.reshape(-1,2)[ele3.reshape(-1),:]
        #         for p in pts:
        #             cv2.circle(img, [int(p[0]), int(p[1])], 1, 255, -1)

        #         ele3, ele4 = bev_mask[1,batch_id, cam_id, ...], reference_points_cam[1,batch_id, cam_id, ...]
        #         pts = ele4.reshape(-1,2)[ele3.reshape(-1),:]
        #         for p in pts:
        #             cv2.circle(img, [int(p[0]), int(p[1])], 1, 127, -1)
        ...
        #         cv2.imwrite(f"temp/{ele_i}.jpg", img)
        #         ele_i+=1
        #         # exit(1)
    ...
    def forward(
        ...
        for k in feats:
            for fea in feats[k]:
                ...
                B, C, H, W = fea.shape
        feats = torch.stack(image_feats_stack, dim=1).reshape(-1, C, H, W)
        ...
        if not self.is_compile:
            ...
            # ref2d_vis = ref2d[0].squeeze()
            # ref2d_vis[:,0] *= 20
            # ref2d_vis[:,1] *= 10
            # vis = Vis2D([-30, 100], [-40, 40], 0.2)
            # vis.DrawPointcloud(ref2d_vis.cpu().numpy()[20:], [0,255,0])
            # vis.DrawPointcloud(ref2d_vis.cpu().numpy()[:20], [255,0,0])
            # vis_draw = vis.Draw()
            # cv2.imwrite("ref2d_vis.jpg", vis_draw)

            # ref3d_vis = ref3d[0,0].reshape(-1,3)
            # vis = Vis2D([-30, 100], [-40, 40], 0.2)
            # vis.DrawPointcloud(ref3d_vis.cpu().numpy()[100:], [0,255,0])
            # vis.DrawPointcloud(ref3d_vis.cpu().numpy()[:100], [255,0,0])
            # vis_draw = vis.Draw()
            # cv2.imwrite("ref3d_vis.jpg", vis_draw)
