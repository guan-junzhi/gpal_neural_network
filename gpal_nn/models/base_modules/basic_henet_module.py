...
from gpal_nn.models.base_modules.channelscale2d import ChannelScale2d
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        query = self.query(x)
        key = self.key(x)
        query_weight = torch.matmul(query, self.query_weight)
        query_weight = torch.sum(query_weight, self.scale)
        attn_weight = torch.sum(
            torch.sum(query_weight, query), dim=1, keepdim=True
        )
        attn_weight = attn_weight.repeat(1, key.shape[1], 1)

        out = torch.add(
            self.projection(torch.mul(attn_weight, key)), query
        )
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale and isinstance(self.attn, LinearAttention):
            ...
            x = torch.add(
                x,
                torch.mul(
                    self.layer_scale_mlp, self.mlp(x)
                ),
            )
            ...
            x = torch.add(
                x,
                torch.mul(
                    self.layer_scale_attn, self.attn(x)
                ),
            )
            x = torch.add(
                x,
...
class DWCB(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale:
            x = torch.add(input, self.layer_scale(x))
        else:
            x = torch.add(input, x)
        x = self.extra_act(x)
        return x
