...
class DRIVING_BEV_STAPostProcessing(BasePostProcess):
    ...
    def process_pred(self, vectors, metadata: dict) -> dict:
        ...
        out_k = ['all_cls_scores', 'all_pts_preds', 
                 ...
                 'all_shape_types_preds', 'all_centerline_types_preds', 'all_centerline_directions_preds',
                 'all_keypoint_classes_preds', 'all_keypoint_regs_preds', 'all_polygon_classes_preds', 'all_arrow_classes_preds']
        ...
        if 'onnx_path' in self.global_config.config and ('.bc' in onnx_path  or '.onnx' in onnx_path or '.hbm' in onnx_path):
            ...
            if 'lane_3d_output' in _vectors:
                outputs_classes, intermediate_reference_points, lane_marking_types, lane_marking_colors, shape_types, centerline_types, centerline_directions, keypoint_classes, keypoint_regs, polygon_classes, arrow_classes = _vectors['lane_3d_output']
                cls_pred, points_pred, lane_marking_type_pred, lane_marking_color_pred, shape_type_pred, centerline_type_pred, centerline_direction_pred, keypoint_cls_pred, keypoint_reg_pred, polygon_class_pred, arrow_class_pred = outputs_classes[self.num_decode_layer - 1], \
                                     ...
                                     shape_types[self.num_decode_layer - 1], centerline_types[self.num_decode_layer - 1],  centerline_directions[self.num_decode_layer - 1], \
                                     keypoint_classes[self.num_decode_layer - 1], \
                                     keypoint_regs[self.num_decode_layer - 1], polygon_classes[self.num_decode_layer - 1], arrow_classes[self.num_decode_layer - 1]
                # bbox_pred, points_pred = self.lane_map_head.transform_box(points_pred)
                outputs['lane_3d_output'] = cls_pred, points_pred, lane_marking_type_pred, lane_marking_color_pred, shape_type_pred, centerline_type_pred, centerline_direction_pred, keypoint_cls_pred, keypoint_reg_pred, polygon_class_pred, arrow_class_pred
        ...
        outputs2["static_3d_pred"]["lane_3d_output"] = (
            outputs2["static_3d_pred"]["lane_3d_output"][0], None, outputs2["static_3d_pred"]["lane_3d_output"][1], outputs2["static_3d_pred"]["lane_3d_output"][2],
              ...
              outputs2["static_3d_pred"]["lane_3d_output"][6], outputs2["static_3d_pred"]["lane_3d_output"][7], outputs2["static_3d_pred"]["lane_3d_output"][8],
              outputs2["static_3d_pred"]["lane_3d_output"][9], outputs2["static_3d_pred"]["lane_3d_output"][10])
        cls_pred, bbox_pred, points_pred, lane_marking_types_pred, lane_marking_colors_pred, shape_types_pred, centerline_types_pred, centerline_directions_pred, \
            keypoint_cls_pred, keypoint_reg_pred, polygon_classes_pred, arrow_classes_pred = outputs2['static_3d_pred']['lane_3d_output']
        ...
        for idx in range(len(points_pred)):
            ...
            centerline_directions_per_pred = centerline_directions_pred[idx]
            # keypoint_cls_per_pred = keypoint_cls_pred[idx]
            # keypoint_reg_per_pred =keypoint_reg_pred[idx]
            polygon_classes_per_pred = polygon_classes_pred[idx]
            arrow_classes_per_pred = arrow_classes_pred[idx]

            # print(self.pc_range)
            # print(point_per_pred)
            cls_per_pred = cls_pred[idx]
            bbox_per_pred, point_per_pred, score_per_pred, type_per_pred, lane_marking_type_per_pred, lane_marking_color_per_pred, \
                shape_type_per_pred, centerline_type_per_pred, centerline_direction_per_pred, keypoint_cls_pred, keypoint_reg_pred, \
                polygon_class_per_pred, arrow_class_per_pred  = decode_pred_with_score(cls_per_pred, bbox_per_pred, point_per_pred, \
                lane_marking_types_per_pred, lane_marking_colors_per_pred, shape_types_per_pred, centerline_types_per_pred, \
                centerline_directions_per_pred, polygon_class_pred=polygon_classes_per_pred, arrow_class_pred=arrow_classes_per_pred,\
                pc_range=self.pc_range, num_query=self.num_vec)
            ...
            for idx, item in enumerate(zip(point_per_pred, type_per_pred, lane_marking_type_per_pred, lane_marking_color_per_pred, 
                    shape_type_per_pred, centerline_type_per_pred, centerline_direction_per_pred, polygon_class_per_pred, arrow_class_per_pred)):
                static_target = {
                    'pts': item[0][..., :2].tolist(),
                    ...
                    'centerline_direction': item[6].item(),
                    'polygon_class': item[7].item(),
                    'arrow_class': item[8].item(),
                }
    ...
    def process_gt(self, vectors, metadata: dict):
        ...
        for idx, data in enumerate(vectors):
            result = dict()
            result['gt_vectors'] = []
            points, cls, lane_marking_types, lane_marking_colors, shape_types, centerline_types, centerline_directions,\
                is_split_merges, keypoint_norms, polygon_classes, arrow_classes = pack_polyline_gt_points(data)

            # points = shift_lane_points(points, self.pts_per_vector)
            if self.is_set_gt_z_as_zero == True and isinstance(points, np.ndarray):
                points[..., 2] = 0.0

            for index, item in enumerate(zip(points, cls, lane_marking_types, lane_marking_colors, \
                    shape_types, centerline_types, centerline_directions, polygon_classes, arrow_classes)):
                static_target = {
                    'pts': item[0].tolist(),
                    'type': item[1],
                    ...
                    'centerline_direction': item[6],
                    'polygon_class': item[7],
                    'arrow_class': item[8],
                }
