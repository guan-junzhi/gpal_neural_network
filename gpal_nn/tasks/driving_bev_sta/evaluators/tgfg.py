...
def tpfp_gen(gen_lines,
    ...
    if num_gts == 0:
        fp[...] = 1
        return tp, fp, dist_error_list, lane_marking_type_acc, lane_marking_color_acc, shape_type_acc, pred_shapes, true_shapes, centerline_type_acc, centerline_direction_acc
    ...
    matrix = polyline_score(
            gen_lines[:,:-6].reshape(num_gens,-1,coord_dim), 
            gt_lines[:,:-5].reshape(num_gts,-1,coord_dim),linewidth=2.,metric=metric)
    ...
    for pred_idx, gt_idx in zip(row_ind, col_ind):
        if matrix[pred_idx, gt_idx] >= threshold:
            tp[pred_idx] = 1
            fp[pred_idx] = 0
            dist_error = get_dis_error(gen_lines[pred_idx,:-6].reshape(-1, 2), gt_lines[gt_idx,:-5].reshape(-1, 2))
            dist_error_list.append(dist_error)
            # shape_type_acc.append(gen_lines[pred_idx,-1] == gt_lines[gt_idx,-1])
            pred_lane_marking_type = gen_lines[pred_idx, -5]  # 预测的车道线类别
            true_lane_marking_type = gt_lines[gt_idx, -5]     # 真实的车道线类别
            lane_marking_type_acc.append(pred_lane_marking_type == true_lane_marking_type)
            pred_lane_marking_color = gen_lines[pred_idx, -4]  # 预测的车道线颜色
            true_lane_marking_color = gt_lines[gt_idx, -4]     # 真实的车道线颜色
            lane_marking_color_acc.append(pred_lane_marking_color == true_lane_marking_color)

            # 获取形状类型并判断是否正确
            pred_shape = gen_lines[pred_idx, -3]  # 预测的形状类型
            true_shape = gt_lines[gt_idx, -3]     # 真实的形状类型
            ...
            # 获取中心线类型并判断是否正确
            pred_centerline_type = gen_lines[pred_idx, -2]  # 预测的形状类型
            true_centerline_type = gt_lines[gt_idx, -2]     # 真实的形状类型
            centerline_type_acc.append(pred_centerline_type == true_centerline_type)

            # 获取中心线方向并判断是否正确
            pred_centerline_direction = gen_lines[pred_idx, -1]  # 预测的中心线方向
            # print("pred_centerline_direction", pred_centerline_direction)
            true_centerline_direction = gt_lines[gt_idx, -1]     # 真实的中心线方向
