...
class BaseDataset(Dataset, ABC):
    ...
    def _dataset_logging(self):
        ...
        logging.info("%s: %s has %s %s labels", self.task_name, self.dataset_name, len(self.dataset), self.phase)
    ...
    def _distribute_data(self):
        ...
        if self.global_config.debug:
            ...
            start_index = size_remain * (size_per_rank + 1) + (rank_curr - size_remain) * size_per_rank
            end_index = start_index + size_per_rank
        dataset = pickle.loads(pickle.dumps(self.world_data_list[start_index: end_index]))
        logging.info("Distirbuted dataset completed: rank %d, world_size %d, data %d.", rank_curr, world_size, len(dataset))
            ...
            try:
                ...
                for key, val in data_blob.items():
                    if key == const.METADATA:
                        processed_data_blob[key] = json.dumps(metadata, cls=JsonSerializer)
                    else:
