import logging
from zpilot_lightning.neural_network.tasks.base.datasets.image_base_dataset import ImageBaseDataset
...
class VideoBaseDataset(ImageBaseDataset):
    ...
    def _dataset_logging(self):
        logging.info("%s: %s has %s %s video labels", self.task_name, self.dataset_name, len(self.dataset), self.phase)
